...
/* ---- offline content bundle ----
   A file:// page cannot fetch sibling files, but it can load a script. Emitting
   the text as JS is what lets the reader work with no server at all. Notebook
   outputs are slimmed hard: base64 images (which hide in cell attachments as
   well as outputs) and rendered HTML are the difference between 3 MB and 63 MB,
   and the reader truncates long text output anyway. */
const CAP = 8000;
function capText(str) {
  return str.length > CAP ? str.slice(0, CAP) + '\n…[truncated in the offline bundle]' : str;
}
function slimNotebook(raw) {
  try {
    const nb = JSON.parse(raw);
    delete nb.metadata;
    (nb.cells || []).forEach((c) => {
      delete c.metadata; delete c.id;
      delete c.attachments;
      (c.outputs || []).forEach((o) => {
        if (o.data) {
          Object.keys(o.data).forEach((k) => {
            if (k.indexOf('image/') === 0 || k.indexOf('widget') >= 0 || k === 'text/html') delete o.data[k];
            else o.data[k] = [capText([].concat(o.data[k]).join(''))];
          });
        }
        if (o.text) o.text = [capText([].concat(o.text).join(''))];
      });
    });
...
const content = {};
let bundled = 0;
courses.forEach((c) => c.sections.forEach((s) => s.items.forEach((i) => {
  let raw;
  try { raw = fs.readFileSync(path.join(ROOT, i.path), 'utf8'); } catch { return; }
  content[i.path] = i.path.endsWith('.ipynb') ? slimNotebook(raw) : raw;
  bundled++;
})));
const CONTENT_OUT = path.join(ROOT, 'library-content.js');
fs.writeFileSync(CONTENT_OUT,
  '/* Generated by build-index.js. Local use only — do not commit or redistribute:\n' +
  "   this is other people's course material under their own licences. */\n" +
  'window.LIBRARY_CONTENT = ' + JSON.stringify(content) + ';\n');
...
console.log(`  ${bundled} files bundled for offline reading ` +
  `(${(fs.statSync(CONTENT_OUT).size / 1048576).toFixed(1)} MB)`);
